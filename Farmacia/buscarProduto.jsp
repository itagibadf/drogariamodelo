                ...
                <a href="PedidoServlet?cmd=listarPedidosUsuario&idUsuario=${user.idUsuario}" >    
                    <c:choose>
                        <c:when test="${contador != null}">
                            <div id class="imgCarrinho" title="Você possui ${contador} item(s) em seu carrinho valor Total:<fmt:formatNumber type="currency" value="${total}"/>. Clique e veja.">
        ...
        <div id="conteudo">
            ...
            <div id="conteudoProdutos">
                <c:if test="${nomeBusca == ''}">
                    ...
                    <h3 style="margin-left: 22%">Informe um Medicamento válido para ser buscado!.</h3>
                </c:if>
                <c:if test="${nomeBusca != ''}">
                    <c:forEach var="produto" items="${ requestScope.produtoList }" varStatus="count">
                        ...
                        <c:if test="${idRemedio == null}">
                            <script>alert('Produto não encontrado, verifique se o nome do medicamento foi digitado corretamente');</script>
