...
<html>
    ...
    <body>
        ...
        <div id="conteudo" style="overflow: auto;">
            <div id class="titulo"style="left: 0.4%"><a href="index.jsp">Drogaria Pharmaluci</a> - <a href="logadoGerente.jsp">Início</a> - <a href="#"><b>Pedidos Negados</b></a></div>
            <a href="PedidoServlet?cmd=listarPedidosPendentes"><font style="float: left; color:blue;font-size: 14pt; margin-left: 0.5%; margin-top: 8.4%">&raquoPendentes</font></a>
            <a href="PedidoServlet?cmd=todosPedidosLiberados"><font style="float: left; color: green; font-size: 14pt; margin-left: 0.5%; margin-top: 8.4%">&raquoLiberados</font></a>
            <table>
                <tr>
                    <th>Nome</th>
                    <th>Sobrenome</th>
                ...
                <c:forEach var="pedido" items="${ requestScope.pedidoList}">
                    <tr>
                        <td><c:out default="nome" value="${pedido.usuario.nome}"/></td>
                        <td><c:out default="sobrenome" value="${pedido.usuario.sobrenome}"/></td>

                        <td><c:out default="dataPedido" value="${pedido.dataPedido}"/></td>
                        <td><c:out default="necessitaEntrega" value="${pedido.necessitaEntrega}"/></td>
                        <td><c:out default="enderecoEntrega" value="${pedido.enderecoEntrega}"/></td>
                        <td>${pedido.estado}
                            <c:choose>
                        ...
                        <c:set var="idUsuario" value="${pedido.usuario.idUsuario}"/>


                        <td><!--Usuário Servlet que puxa os campos dos usuários para serem atualizados-->
                            <!--                        <form method="POST" action="PedidoServlet?cmd=listarPedidosUsuario">
                                                        <input type="hidden" name="idUsuario" value="${pedido.usuario.idUsuario}">-->
                            <form method="POST" action="PedidoServlet?cmd=listarPedidosUsuario&idUsuario=${pedido.usuario.idUsuario}"><input type="submit" value="Edit"></a>
                            </form>
                        </td>
                        <td>
                            <form action="PedidoServlet?cmd=excTodosPedidosUsuario&idUsuario=${idUsuario}" method="POST"><input type="submit" value="Del">
                            </form>
                        </td>
                    </c:forEach>
            </table>
            <c:if test="${idUsuario == null}">
                <hr size="2"  style="background:#d7d7d9;">
                <font color="black" style=" font-size: 13pt; text-align: left;"><b>&raquoNão há pedidos Negados no momento</b></font>
                <hr size="2"  style="background:#d7d7d9;">
            </c:if>
        </div>
        <div id="rodape" style="min-height: 335px;">
            <div class="table">
                <h3> Menu</h3>
