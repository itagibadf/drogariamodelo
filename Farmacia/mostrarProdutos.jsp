...
<html>
    ...
    <body>
        ...
        <div id="conteudo" style="overflow: auto;">
            <div id class="titulo" style="left: 0.5%"><a href="index.jsp">Drogaria Pharmaluci</a> - <a href="logadoGerente.jsp">Início</a> - <a href="#"><b>Mostrar Produtos</b></a></div>
            ${mensagem}
            <a href="formInserindoProduto.jsp"><input type="submit" value="+"></a>
            ...
            <table style="margin-top: 0%;">
                ...
                <c:forEach var="lista" items="${ requestScope.produtoList }">
                    ...
                    <tr>
                        <c:set var="img" value=""/>
                        <td>${lista.idRemedio}</td>
                        <td><a href="ProdutoServlet?cmd=mostraProduto&idRemedio=${lista.idRemedio}"><img src="imagem/Produtos/${lista.idRemedio}.jpg" width="100%" height="100px"></a></td>
                        <td>${lista.nome}</td>
                        <td><div class="precoAntigo">De <fmt:formatNumber type="currency" value="${lista.precoAntigo}"/></div><p>
                            Por <fmt:formatNumber type="currency" value="${lista.preco}"/></td>
                        <td><fmt:formatNumber maxFractionDigits="0">${lista.quantidadeEstoque}</fmt:formatNumber></td>
                        <td><fmt:formatNumber maxFractionDigits="0">${lista.medida}</fmt:formatNumber></td>
                        <td>${lista.unidadeMedida}</td>
                        <td>${lista.validade}</td>
                        <td>${lista.categoria}</td>
                        <td>${lista.promocao}</td>
                        <c:set var="idRemedio" value="${lista.idRemedio}"/>
                        <td>
                            <!--                    Usuário Servlet que puxa os campos dos usuários para serem atualizados-->
                            <a href="ProdutoServlet?cmd=atu&idRemedio=${lista.idRemedio}"><input type="submit" value="Edit" > </a>


                        </td>
                        <td>
                            <form method="POST" action="ProdutoServlet?cmd=exc&idRemedio=${lista.idRemedio}"><input type="submit" value="Del">
                            </form>
                            </a>
                        </td>
                    </tr>
                    ...
                    <form action="ProdutoServlet?cmd=procurarProdutoLista" method="POST">
                    <input placeholder="Pesquise" name="nomeBusca" required title="Digite o nome que procura" value="${nomeBusca}" style="float: left;width: 14%; margin-left: 11%;  margin-top: 8.1%"> <input type="submit" value="Buscar" style="position: relative;margin-top: 8.1%; left:44.4%; width: 6%; height: 30px;">
                    </form>
                   
                    </table>
                     <c:if test="${idRemedio == null}">
                         <font style="font-size:14pt; margin-left: 1%;">Produto não encontrado, por favor verifique se o nome digitado está correto.</font>
                    </c:if>
        </div>
        <div id="rodape">
            <div class="table">
